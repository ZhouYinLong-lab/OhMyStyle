...
def validate_package(package: Path, schema: dict[str, Any], errors: list[str]) -> None:
    ...
    try:
        data = load_yaml(package / "package.yaml")
    ...
    try:
        with (package / "references/manifest.csv").open(encoding="utf-8", newline="") as handle:
            reader = csv.DictReader(handle)
            columns = set(reader.fieldnames or [])
            for missing in sorted(SOURCE_COLUMNS - columns):
                errors.append(f"{package / 'references/manifest.csv'}: missing column {missing}")
            for row_number, row in enumerate(reader, start=2):
                for field in ("asset_id", "title", "creator", "source_url", "license", "attribution", "role"):
                    if not row.get(field, "").strip():
                        errors.append(f"{package / 'references/manifest.csv'}:{row_number}: {field} is required")
                local_path = row.get("local_path", "").strip()
                if local_path and not (package / local_path).is_file():
                    errors.append(f"{package / 'references/manifest.csv'}:{row_number}: missing local asset {local_path}")
    except OSError as exc:
        errors.append(f"{package / 'references/manifest.csv'}: unreadable: {exc}")

    base_prompt = (package / "prompts/base.txt").read_text(encoding="utf-8").lower()
