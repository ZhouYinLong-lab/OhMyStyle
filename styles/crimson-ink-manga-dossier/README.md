...
![Crimson Ink Manga Dossier representative example](preview-16x9.jpg)


> **分类 / Category:** [继承的 110 个轻量预设 / Inherited 110 lightweight presets]
> **类型 / Type:** 继承的轻量预设 / inherited lightweight preset
> **包路径 / Package path:** `styles/crimson-ink-manga-dossier`

## 简介（中文）

这是从原始 `AI-Visual-Prompt-Cookbook` 继承的轻量风格预设「Crimson Ink Manga Dossier」。本仓库保留原有 `style.json`、预览图与兼容目录；此 README 只提供导航、使用方式和归属说明，不将继承内容重新宣称为 OhMyStyle 原创。

核心观察点：One dominant hand-drawn manga character occupies roughly two thirds of the frame and breaks across surrounding editorial panels.；Dramatic low-angle or close wide-angle perspective with strong foreground foreshortening and a confident upward gaze.；Oversized uppercase condensed sans-serif headline spans the upper field, partially hidden behind the hero and cropped by the edges.；Dense asymmetrical magazine-newspaper layout with narrow side columns, boxed dossiers, pull quotes, captions, and small comic insets.

## Overview (English)

This is the inherited lightweight preset “Crimson Ink Manga Dossier” from the original `AI-Visual-Prompt-Cookbook` catalog. OhMyStyle keeps its `style.json`, previews, and compatibility path; this README adds navigation and usage guidance without claiming authorship.

Key observations: One dominant hand-drawn manga character occupies roughly two thirds of the frame and breaks across surrounding editorial panels.; Dramatic low-angle or close wide-angle perspective with strong foreground foreshortening and a confident upward gaze.; Oversized uppercase condensed sans-serif headline spans the upper field, partially hidden behind the hero and cropped by the edges.; Dense asymmetrical magazine-newspaper layout with narrow side columns, boxed dossiers, pull quotes, captions, and small comic insets.
...
下载本目录，打开 `styles/crimson-ink-manga-dossier/style.json`，或查看 [`docs/copy-prompts/crimson-ink-manga-dossier.md`](../../docs/copy-prompts/crimson-ink-manga-dossier.md)。 把包内变量替换为你的主题，再将生成的 Prompt 复制到任意支持文字生图的平台。参考图、负面约束和可选参数见同目录文件。

Download this directory. Open `styles/crimson-ink-manga-dossier/style.json`, or read [`docs/copy-prompts/crimson-ink-manga-dossier.md`](../../docs/copy-prompts/crimson-ink-manga-dossier.md). Fill in the variables and paste the resulting Prompt into an image model. Reference roles, negative constraints, and optional parameters are kept beside the package.
