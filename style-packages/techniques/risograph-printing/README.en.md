...
![Risograph Printing representative](gallery-16x9.jpg)

> **Category:** technique
> **Domain:** printmaking
> **Path:** `style-packages/techniques/risograph-printing`

## Overview

以有限专色、粗网点、纸张肌理和轻微套印偏移组织画面；本包描述印刷过程与表面结果，不固定海报、静物或文字内容。
...
Risograph printing is compelling because it is limited: color is separated into a few spot plates, tone is built from dots, small registration errors remain visible, and the paper becomes part of the image. The representative still life keeps the subject ordinary so the process—plate order, coverage, and paper tooth—stays easy to inspect instead of turning into a fixed content recipe.
