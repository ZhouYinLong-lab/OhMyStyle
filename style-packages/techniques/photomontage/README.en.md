...
![Photomontage representative](gallery-16x9.jpg)

> **Category:** technique
> **Domain:** hybrid
> **Path:** `style-packages/techniques/photomontage`

## Overview

以摄影片段、遮罩、负片叠合、硬边几何和尺度错置构成新的平面空间；本包描述蒙太奇的构成机制，不固定人物、建筑或叙事。
...
Photomontage becomes interesting when it refuses to disguise different sources as one seamless world. A hand, an architectural fragment, a machine, or a landscape may retain its own texture and scale. What holds them together is the cut edge, the mask, and a clear visual direction. The result carries the trace of paper and darkroom experiment without requiring a nostalgia filter. Start by deciding which fragments carry information, then decide what can be removed.
