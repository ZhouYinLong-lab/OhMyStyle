...
![巴勃罗·毕加索代表图](gallery-16x9.jpg)

> **分类：** 艺术家
> **类型：** 艺术家
> **目录：** `style-packages/artists/pablo-picasso`

## 简介

这是一个面向「巴勃罗·毕加索」的独立风格包。它把公开作品、研究资料和可观察的媒介、构图、色彩、光线、材质与纹理决策整理为可执行约束，用于生成新的主题，不用于复制某一幅具体作品。
...
分析性平面、多视点、结构化静物与几何重组。Prompt 会优先表达这些可观察特征，同时保留新主题、新构图和新的对象关系。

完整规则见 `visual-signature.yaml`、`reproduction.yaml`、`palette/palette.json` 和 `evaluation.yaml`。

## 参考来源

- [Pablo Picasso / 现代主义](https://commons.wikimedia.org/w/index.php?search=Pablo+Picasso&title=Special:MediaSearch&type=image)
