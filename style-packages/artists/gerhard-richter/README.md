...
![格哈德·里希特代表图](gallery-16x9.jpg)

> **分类：** 艺术家
> **媒介领域：** 绘画
> **目录：** `style-packages/artists/gerhard-richter`

## 简介

我喜欢这类画面里“看得见，又没有完全看清”的状态：主体没有消失，表面却不断把它推远。灰蓝底色让画面先安静下来，横向刮擦和擦除再把观看变成一种来回确认。

这是一个可独立使用的风格包。它把可观察的媒介、构图、光线、色彩、表面、纹理和复现步骤整理成可执行规则，用于生成新的主题，不用于复制某一件具体作品。

## 策展短评

这个包最适合用来处理“识别与不确定”之间的张力。先让主体站稳，再用拖拽、擦除和半透明覆盖打断它；如果一开始就做成纯抽象，反而失去了它最有意思的部分。
...
参考资料只用于研究和分析。外部作品、商标和网页内容仍归原权利人所有。生成示例是新的匿名场景，不是艺术家的原作，也不代表合作、授权或背书关系。

详细来源和再分发边界见 [`provenance.yaml`](provenance.yaml)、[`references/manifest.csv`](references/manifest.csv) 以及仓库根目录的 [`NOTICE`](../../../NOTICE)。代表图中的花瓶和衣物只是测试主体。
