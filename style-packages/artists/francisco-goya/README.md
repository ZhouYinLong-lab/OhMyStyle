...
![弗朗西斯科·戈雅代表图](gallery-16x9.jpg)

> **分类：** 艺术家
> **类型：** 艺术家
> **目录：** `style-packages/artists/francisco-goya`

## 简介

这是一个面向「弗朗西斯科·戈雅」的独立风格包。它把公开作品、研究资料和可观察的媒介、构图、色彩、光线、材质与纹理决策整理为可执行约束，用于生成新的主题，不用于复制某一幅具体作品。

## 策展短评

我会把new subject and arrangement；clear primary focal hierarchy看作这个包的入口。它带来的观看距离很明确，题材不需要固定。

## 风格重点

心理张力、暗色现实、粗粝笔触与不安静默。Prompt 会优先表达这些可观察特征，同时保留新主题、新构图和新的对象关系。

完整规则见 `visual-signature.yaml`、`reproduction.yaml`、`palette/palette.json` 和 `evaluation.yaml`。

## 参考来源

- [Francisco Goya / 浪漫主义前夜](https://commons.wikimedia.org/w/index.php?search=Francisco+Goya&title=Special:MediaSearch&type=image)
