...
![莉莲·巴斯曼 代表图](gallery-16x9.jpg)

> **分类：** photographer
> **媒介领域：** photography
> **目录：** style-packages/photographers/lillian-bassman

## 简介

以时装摄影为入口，用柔焦、暗房擦拭、局部反差和溶解的轮廓把衣物转化为空气与姿态。

这是一个可独立使用的风格包。它提取可观察的视觉语言，用来处理用户指定的新主题，不会把代表图中的物体、地点、人物或故事写入默认生成结果。

## 策展短评

它像一张被暗房重新编辑过的照片：衣物、身体和光线被压缩成几条会呼吸的轮廓。不要把它用成全图柔焦，选择性控制才是重点。
