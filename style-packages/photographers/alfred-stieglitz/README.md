...
![Alfred Stieglitz representative example](examples/accepted/anonymous-v1.png)


> **分类 / Category:** [摄影师 / Photographers]
> **类型 / Type:** 摄影师 / photographer
> **包路径 / Package path:** `style-packages/photographers/alfred-stieglitz`

## 简介（中文）

这是一个面向「Alfred Stieglitz」的独立 摄影师。它把公开作品、研究资料和可观察视觉特征整理成可执行约束，重点关注 `camera, composition, lighting, palette, surface, subject_treatment`，用于生成新场景，而不是复制某一幅原作。

核心观察点：construct the frame from intersecting planes, rails, ropes, steps, trunks, or crowd bands；use asymmetry and cropped structure to make the frame feel authored；create depth through overlapping flat zones rather than cinematic bokeh；restrained monochrome or warm photogravure brown

## Overview (English)

This is an independent photographer package for “Alfred Stieglitz”. It turns public references, research material, and observable visual decisions into executable constraints focused on `camera, composition, lighting, palette, surface, subject_treatment`. It is intended for new scenes, not copies of a named artwork.

Key observations: construct the frame from intersecting planes, rails, ropes, steps, trunks, or crowd bands; use asymmetry and cropped structure to make the frame feel authored; create depth through overlapping flat zones rather than cinematic bokeh; restrained monochrome or warm photogravure brown
...
- [https://commons.wikimedia.org/wiki/File:Alfred_Stieglitz_-_The_Steerage_-_Google_Art_Project,_from_Getty.jpg](https://commons.wikimedia.org/wiki/File:Alfred_Stieglitz_-_The_Steerage_-_Google_Art_Project,_from_Getty.jpg)
- [https://www.metmuseum.org/art/collection/search/267836](https://www.metmuseum.org/art/collection/search/267836)
...
下载本目录，打开 `style-packages/photographers/alfred-stieglitz/prompts/base.txt`，替换变量后复制。 把包内变量替换为你的主题，再将生成的 Prompt 复制到任意支持文字生图的平台。参考图、负面约束和可选参数见同目录文件。

Download this directory. Open `style-packages/photographers/alfred-stieglitz/prompts/base.txt`, fill in its variables, and paste the result. Fill in the variables and paste the resulting Prompt into an image model. Reference roles, negative constraints, and optional parameters are kept beside the package.
...
```bash
python tools/compile-style.py style-packages/photographers/alfred-stieglitz --subject "你的主题" --profile weak --output tmp/alfred-stieglitz-job.json
```
