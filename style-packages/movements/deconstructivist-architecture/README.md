...
![解构主义建筑代表图](gallery-16x9.jpg)

> **分类：** 艺术流派
> **媒介领域：** 设计
> **目录：** `style-packages/movements/deconstructivist-architecture`

## 简介

解构主义建筑把空间看成一组彼此错位、切割和竞争的体块：倾斜轴线、断裂平面、非中心构图与材料接缝共同制造张力。本包迁移的是这种组织方式，不会把博物馆、尖塔、大桥或任何具体建筑写进每一次生成。

这是一个可独立使用的风格包。它把可观察的媒介、构图、光线、色彩、表面、纹理和复现步骤整理成可执行规则，用于生成新的主题，不用于复制某一件具体作品。

## 策展短评

这张代表图的重点不在“造一座奇怪的建筑”，而在让结构关系保持可读：体块彼此推挤，斜向线条打破平衡，混凝土、玻璃和金属用不同的反光方式确认层次。换成家具、室内或城市空间时，仍应保留这种有秩序的错位感。
