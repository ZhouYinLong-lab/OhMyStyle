...
![Greek Archaic Period representative example](examples/generated/anonymous-v1.png)


> **分类 / Category:** [艺术流派与历史时期 / Movements and periods]
> **类型 / Type:** 艺术流派/历史时期 / movement or historical period
> **包路径 / Package path:** `style-packages/movements/greek-archaic-period`

## 简介（中文）

这是一个面向「Greek Archaic Period」的独立 艺术流派/历史时期。它把公开作品、研究资料和可观察视觉特征整理成可执行约束，重点关注 `medium, composition, lighting, palette, surface, texture`，用于生成新场景，而不是复制某一幅原作。

核心观察点：build a stable field around a central action or procession；fill negative space with controlled rosettes, lotuses, or small marks；use profile figures and overlapping limbs to clarify narrative；black or deep brown

## Overview (English)

This is an independent movement or historical period package for “Greek Archaic Period”. It turns public references, research material, and observable visual decisions into executable constraints focused on `medium, composition, lighting, palette, surface, texture`. It is intended for new scenes, not copies of a named artwork.

Key observations: build a stable field around a central action or procession; fill negative space with controlled rosettes, lotuses, or small marks; use profile figures and overlapping limbs to clarify narrative; black or deep brown
...
- [https://www.metmuseum.org/toah/hd/argk/hd_argk.htm](https://www.metmuseum.org/toah/hd/argk/hd_argk.htm)
...
下载本目录，打开 `style-packages/movements/greek-archaic-period/prompts/base.txt`，替换变量后复制。 把包内变量替换为你的主题，再将生成的 Prompt 复制到任意支持文字生图的平台。参考图、负面约束和可选参数见同目录文件。

Download this directory. Open `style-packages/movements/greek-archaic-period/prompts/base.txt`, fill in its variables, and paste the result. Fill in the variables and paste the resulting Prompt into an image model. Reference roles, negative constraints, and optional parameters are kept beside the package.
...
```bash
python tools/compile-style.py style-packages/movements/greek-archaic-period --subject "你的主题" --profile weak --output tmp/greek-archaic-period-job.json
```
