...
![早期三维生存恐怖游戏美术代表图](gallery-16x9.jpg)

> **分类：** 游戏美术
> **媒介领域：** 游戏美术
> **目录：** `style-packages/game-art/ps1-survival-horror-low-poly`

## 简介

以低多边形几何、低分辨率纹理、固定机位、近距离雾化和预计算阴影制造早期三维游戏的紧张空间。

这是一个可独立使用的风格包。它把可观察的媒介、构图、光线、色彩、表面、纹理和复现步骤整理成可执行规则，用于生成新的主题，不用于复制某一件具体作品。

## 策展短评

这类画面最有意思的地方，不是把现实做得更精细，而是让有限的几何、贴图和光线共同承担空间信息。低多边形边缘、固定机位和局部雾化会把一个普通场景变得有距离感；真正值得保留的是这种“看得见限制，却仍然能读懂空间”的秩序。示例使用了空置走廊，只为展示机位、材质和雾化，不代表每次都要生成走廊或恐怖叙事。

## 主体独立性

本包只决定“怎么生成”，不决定“生成什么”。人物、物体、地点、建筑、植物、车辆和叙事由你的 Prompt 决定；本包中的具体场景只属于示例或测试，不会作为默认主体加入生成结果。除非用户明确提出，否则不要自动加入走廊、酒店、宅邸、怪物、血迹、武器或恐怖剧情。
...
参考资料只用于研究和分析。外部作品、游戏画面、商标和平台页面仍归原权利人所有。生成示例是新的匿名场景，不是相关游戏、平台或权利人的原作，也不代表合作、授权或背书关系。
