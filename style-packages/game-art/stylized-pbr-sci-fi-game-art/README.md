...
![风格化物理渲染科幻游戏美术 代表图](gallery-16x9.jpg)

> **分类：** 游戏美术
> **媒介领域：** 游戏美术
> **目录：** `style-packages/game-art/stylized-pbr-sci-fi-game-art`

## 简介

在真实材质响应与高度风格化的科幻环境之间建立平衡，使用可读的金属、玻璃、湿地和霓虹反射，但避免把画面退化成无层次的赛博滤镜。
...
模型权重、API Key 和生成图片由使用者自行管理。参考图只用于理解可观察特征，不要复制原作的具体构图、人物、文字、商标或标志。## 策展短评

我会先看“在真实材质响应与高度风格化的科幻环境之间建立平衡，使用可读的金属、玻璃、湿地和霓虹反射，但避免把画面退化成无层次的赛博滤镜”。它把观看节奏定下来，题材可以按你的需要替换。
