# ZX Spectrum Attribute Pixel Art

![ZX Spectrum Attribute Pixel Art representative example](examples/generated/anonymous-v1.png)


> **分类 / Category:** [游戏美术 / Game art]
> **类型 / Type:** 游戏美术方向 / game-art direction
> **包路径 / Package path:** `style-packages/game-art/zx-spectrum-attribute-pixel`

## 简介（中文）

这是一个面向「ZX Spectrum Attribute Pixel Art」的独立 游戏美术方向。它把公开作品、研究资料和可观察视觉特征整理成可执行约束，重点关注 `medium, composition, lighting, palette, texture, layout`，用于生成新场景，而不是复制某一幅原作。

核心观察点：gameplay_first_legibility；strong_horizontal_ground_or_platform；clustered_cells_for_objects；deliberate_empty_border_or_ui_band

## Overview (English)

This is an independent game-art direction package for “ZX Spectrum Attribute Pixel Art”. It turns public references, research material, and observable visual decisions into executable constraints focused on `medium, composition, lighting, palette, texture, layout`. It is intended for new scenes, not copies of a named artwork.

Key observations: gameplay_first_legibility; strong_horizontal_ground_or_platform; clustered_cells_for_objects; deliberate_empty_border_or_ui_band
...
- [https://rk.nvg.ntnu.no/sinclair/computers/zxspectrum/spec_technical.htm](https://rk.nvg.ntnu.no/sinclair/computers/zxspectrum/spec_technical.htm)
...
下载本目录，打开 `style-packages/game-art/zx-spectrum-attribute-pixel/prompts/base.txt`，替换变量后复制。 把包内变量替换为你的主题，再将生成的 Prompt 复制到任意支持文字生图的平台。参考图、负面约束和可选参数见同目录文件。

Download this directory. Open `style-packages/game-art/zx-spectrum-attribute-pixel/prompts/base.txt`, fill in its variables, and paste the result. Fill in the variables and paste the resulting Prompt into an image model. Reference roles, negative constraints, and optional parameters are kept beside the package.
...
```bash
python tools/compile-style.py style-packages/game-art/zx-spectrum-attribute-pixel --subject "你的主题" --profile weak --output tmp/zx-spectrum-attribute-pixel-job.json
```
